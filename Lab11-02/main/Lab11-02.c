...
void app_main(void)
{
    ...
    while (1) {
        ...
        // แปลงเป็นระดับแสง (%)
        float lightLevel = (adc_reading / 4095.0f) * 100.0f;
        ...
        // แสดงผลลัพธ์
        //ESP_LOGI(TAG, "ADC:%" PRIu32 " | แรงดัน:%.2fV | ระดับแสง:%.1f%% | สถานะ:%s",
        //        adc_reading, voltage, lightLevel, lightStatus);

        // สำรองใช้ printf เผื่อ log ถูกปิด
        printf(">LDR1:%.2fV | Light:%.1f%% | สถานะ:%s\n",
               voltage, lightLevel, lightStatus);

        vTaskDelay(pdMS_TO_TICKS(500));  // หน่วงเวลา 500 ms
    }
}
